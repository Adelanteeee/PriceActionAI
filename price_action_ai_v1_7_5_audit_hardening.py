...
def classify_time_gaps(df: pd.DataFrame, timeframe: str) -> dict[str, list[dict[str, Any]]]:
    """Classify wall-clock gaps without changing market logic."""
...
def audit_counts(structural: list[dict], major: list[dict], major_removed: list[dict]) -> dict[str, Any]:
    ...
    removed_set = {
        _pivot_key(item["pivot"])
        for item in major_removed
        if isinstance(item, dict) and "pivot" in item and _pivot_key(item["pivot"]) in structural_set
    }
...
def main() -> None:
    ...
    try:
        ...
        symbol = core.detect_symbol(mt5, args.symbol)
        symbol_info = mt5.symbol_info(symbol)
        spec = symbol_display_spec(symbol, symbol_info)
        full_df = core.fetch_candles(mt5, symbol, timeframe, count)
        gap_result = segment_on_unexpected_gaps(full_df, timeframe)
        active_df = gap_result["active_segment"].copy().reset_index(drop=True)

        raw, structural, internal_tags = core.structural_swings(active_df)
        thrusts = core.leg_thrusts(structural)
        status = reference_data_status(thrusts, args.min_reference_legs)

        if status == "OK":
            ref_stats = core.estimate_reference_leg(thrusts)
            major, major_removed = core.select_major_swings(active_df, structural, ref_stats["reference"])
        else:
            ref_stats = {"source": "INSUFFICIENT_DATA", "reference": 0.0, "mean": 0.0, "median": 0.0, "rms": 0.0, "snap_distance": 0.0}
            major, major_removed = [], []

        fig = core.build_chart(active_df, symbol, timeframe, raw, structural, internal_tags, major, major_removed, ref_stats, len(active_df))
        fig.layout.annotations = ()
        median_atr = _median_atr(core, active_df)
        summary = _summary_text(
            symbol=symbol, timeframe=timeframe, full_count=len(full_df), active_count=len(active_df), spec=spec,
            status=status, gap_result=gap_result, raw=raw, structural=structural, internal_tags=internal_tags,
            major=major, major_removed=major_removed, ref_stats=ref_stats, median_atr=median_atr,
        )
        fig.add_annotation(x=0.01, y=0.99, xref="paper", yref="paper", text=summary, showarrow=False,
                           align="left", xanchor="left", yanchor="top", bgcolor="rgba(0,0,0,0.62)", borderpad=6,
                           font=dict(size=11))
        fig.update_layout(title=(f"PriceActionAI {VERSION} | {symbol} | {timeframe} | Snapshot {len(full_df)} | "
                                 f"Active Segment {len(active_df)} | {status}"))
        if spec.get("is_fx"):
        ...
        for g in gap_result["unexpected_gaps"]:
            print(f"  GAP RESET: {g['previous_time']} -> {g['current_time']} | {g['delta']}")
        print(f"Raw pivots              : {len(raw)}")
        print(f"Structural              : {len(structural)}")
        print(f"Major unique            : {count_audit['major_unique']}")
        print(f"Removed unique/events   : {count_audit['removed_unique']}/{count_audit['removed_events']}")
        print(f"Right-edge provisional  : {count_audit['right_edge_provisional']}")
        print(f"Count invariant         : {'OK' if count_audit['invariant_ok'] else 'ERROR'}")
        print(f"Reference raw           : {format_delta(ref_stats['reference'], spec)}")
