...
def test_locked_provenance_and_counts_are_exact():
    assert TASK11_SPEC_PATH == "docs/superpowers/specs/2026-09-01-task11-evidence-review-hypothesis-registration-design.md"
    ...
    assert TASK11_SPEC_LOCK_RECORD_PATH == "docs/superpowers/specs/2026-09-01-task11-evidence-review-hypothesis-registration-design.LOCKED.md"
    assert TASK11_SPEC_LOCK_COMMIT == "a51dc7f76a5c3ea4c8e2929be7859c02c879a2d9"
    assert TASK11_SPEC_LOCK_RECORD_BLOB_SHA == "de87a930b424b0cf9e58e14c8b27dca336954f33"
    assert TASK10_IMPLEMENTATION_COMMIT == "0a780ca95c4e6853bb2530436c6045c54f508e80"
    assert TASK10_PRODUCTION_PACKAGE_FILENAME == "TASK10_PRODUCTION_RUN1.zip"
...
def test_closed_record_and_locator_schemas_are_exact():
    ...
    assert TASK11_SOURCE_LOCATOR_FIELDS == (
        ...
        "upstream_cross_tf_source_artifact", "upstream_cross_tf_source_row_locator",
    )
...
def test_all_frozen_literals_and_nested_schemas_are_exact():
    assert TIMEFRAMES == ("M5", "M15", "M30", "H1")
    assert TASK10_CANONICAL_PAIR_KEYS == EXPECTED_PAIR_KEYS
    assert TASK10_MEMBER_SHA256_BY_FILENAME == {
    ...
    assert TASK10_MAIN_DOSSIER_FIELDS == (
    ...
    assert TASK10_RAW_TF_FIELDS == (
        "feature_x", "feature_y", "n_missing_x", "n_missing_y", "n_total",
        "n_valid_pairwise", "rho_raw", "status",
    )
    assert TASK10_ELIGIBLE_PARTIAL_TF_FIELDS == (
        "feature_x", "feature_y", "rho_raw_for_delta", "rho_duration_controlled",
        "delta_rho", "n_valid_triple", "status",
    )
    assert TASK10_CONTROL_PARTIAL_TF_FIELDS == (
        "rho_raw_for_delta", "rho_duration_controlled", "delta_rho", "n_valid_triple", "status",
    )
    assert TASK10_CROSS_TF_FIELDS == (
    ...
    assert TASK10_DETERMINISTIC_CONTEXT_FIELDS == (
        "co_participating_relation_ids", "co_participation_semantics",
    )
    assert TASK11_MANIFEST_FIELDS == (
    ...
    assert TASK11_FALSE_SCOPE_FIELDS == (
    ...
    assert TASK11_LOGICAL_FILENAMES == tuple(sorted(TASK11_LOGICAL_FILENAMES))
    assert DETERMINISTIC_RELATION_IDS_BY_PAIR_KEY == {
    ...
    assert LOCKED_STATISTICAL_STATUSES == frozenset({
        "DEFINED", "UNDEFINED_INSUFFICIENT_OBSERVATIONS", "UNDEFINED_CONSTANT_INPUT",
    })
    assert TASK10_MANIFEST_FIELDS == (
    ...
    assert TASK10_MANIFEST_EXPECTED_VALUES == {
        ...
        "supplementary_evidence_row_count": 960, "task": "Task 10 Dependency / Redundancy Interpretation",
